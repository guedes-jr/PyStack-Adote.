...
def ver_pedido_adocao(request):
   if request.method == "GET":
      pedidos = PedidoAdocao.objects.filter(usuario=request.user, status='AG')
...
def api_adocoes_por_raca(request):
   ...
   qtd_adocoes = []
   for raca in racas:
      adocoes = PedidoAdocao.objects.filter(pet__raca=raca, status='AP').count()
